...
def save(request):
    ...
    try:
        mkdir(f'Saves/{charName}') #with mkdi you cannot make a file that already exist, providing us a bit of data protection
    except:
        print(f'directory "Saves/{charName}" was not created, perhaps it was already there')
    
    f = open(f'Saves/{charName}/{charName}.txt', 'w');
    ...
    rf = open(f'Saves/{charName}/{charName}.txt', "r")
    characterString = rf.read()
    rf.close()
